...
Extract a complete, grocery-list-ready recipe from **any URL** - YouTube
videos, Instagram posts, TikTok videos, recipe blogs, or any public webpage.
...
List all URL extractions by the authenticated user (newest first).
...
The frontend can directly map `category` to grocery store aisles for a
...
YouTube URL feature is currently in Gemini **preview at no extra charge**,
so actual costs may be lower.
...
2. **1 credit consumed regardless of outcome** - Even if URL is not a recipe
3. **Single recipe per URL** - If content has multiple recipes, the primary one is extracted
4. **English-centric** - Non-English content may have lower extraction quality
5. **Quantity estimation** - If recipe doesn't state exact quantities, AI estimates (lower confidence)
6. **No caching across users** - Same URL submitted by different users is processed separately

### YouTube-Specific
7. **Public/unlisted only** - Private YouTube videos will fail
8. **8 hours/day limit** - Gemini daily YouTube video processing cap (shared across all users)
...
11. **Webpage text only** - AI reads the page, not embedded videos (no video analysis)
12. **Platform accessibility** - Some platforms may block automated page fetching
13. **Recipe in descriptions** - Quality depends on how much recipe info is in the page text vs. the video itself
...
1. **URL deduplication** - Cache results by normalized URL to avoid re-processing
