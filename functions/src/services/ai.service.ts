...
Be precise and helpful. Return ONLY valid JSON.`;
   ...
   * Public method for standalone recipe generation
   * For image analysis, recommendations are auto-generated
   */
  static async recommendRecipes(
    ingredients: Ingredient[]
  ): Promise<RecipeRecommendationResponse> {
    return this.generateRecipesFromIngredients(ingredients);
  }

  /**
   * Internal method to generate recipe recommendations
   * Used by both standalone API and automatic analysis flow
   */
  private static async generateRecipesFromIngredients(
    ingredients: Ingredient[]
  ): Promise<RecipeRecommendationResponse> {
